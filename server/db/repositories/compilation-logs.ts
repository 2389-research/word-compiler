...
export function getCompilationLog(db: Database.Database, id: string): CompilationLog | null {
  ...
  if (!row) return null;
  return safeJsonParse<CompilationLog>(row.data, "compilation_logs.getCompilationLog");
}
...
export function listCompilationLogs(db: Database.Database, chunkId: string): CompilationLog[] {
  ...
  return rows
    .map((r) => safeJsonParse<CompilationLog>(r.data, "compilation_logs.listCompilationLogs"))
    .filter((l): l is CompilationLog => l !== null);
}
