...
import { IRInspector } from "../../src/app/components/IRInspector.js";
...
describe("IRInspector", () => {
  ...
  it("calls onExtract when Extract IR clicked", () => {
    const onExtract = vi.fn();
    render(<IRInspector {...defaultProps} onExtract={onExtract} />);
    fireEvent.click(screen.getByText("Extract IR"));
    expect(onExtract).toHaveBeenCalledOnce();
  });

  it("calls onClose when ✕ clicked", () => {
    const onClose = vi.fn();
    render(<IRInspector {...defaultProps} onClose={onClose} />);
  ...
  it("calls onVerify when Verify button clicked", () => {
    ...
    render(<IRInspector {...defaultProps} ir={makeIR({ verified: false })} onVerify={onVerify} />);
  ...
  it("renders IR events when IR has data", () => {
    ...
    render(<IRInspector {...defaultProps} ir={ir} />);
